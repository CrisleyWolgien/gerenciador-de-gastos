...
function Expenses() {
  ...
  const handleDelete = async (expenseId) => {
    ...
    const token = localStorage.getItem("token");
    try {
        const response = await fetch(`${backendUrl}/expense/${expenseId}/delete`, {
            method: 'POST',
            headers: { 'Authorization': `Bearer ${token}` }
        });
        if(!response.ok) {
  ...
  const renderContent = () => {
    if (isLoading) {
      return <div className="text-center p-10 text-text-secondary font-mono">// Carregando dados...</div>;
    }
    if (error) {
      return <div className="text-center p-10 text-error font-mono">// ERRO: {error}</div>;
    }
    if (expenses.length === 0) {
      return <div className="text-center p-10 text-text-secondary font-mono">// Nenhuma transação encontrada.</div>;
    }
    return (
      <>
        {/* ======================================================================== */}
        {/* 1. VISÃO DE TABELA PARA DESKTOP (md e acima)                          */}
        {/* ======================================================================== */}
        <table className="hidden md:table w-full text-left font-mono">
          <thead className="border-b border-dark-grid">
            <tr>
              ...
              <th className="p-4 text-sm text-text-secondary uppercase">Transação</th>
          ...
          <tbody>
            {expenses.map((expense) => (
              <tr key={expense.id} className="border-b border-dark-grid last:border-none hover:bg-dark-surface/50">
                <td className="p-4 text-text-primary whitespace-nowrap">{formatarDataSegura(expense.date_created)}</td>
                <td className="p-4">
                  <p className="text-text-primary font-bold">{expense.name}</p>
                  {expense.description && <p className="text-text-secondary text-xs mt-1">{expense.description}</p>}
                </td>
                <td className="p-4 text-text-secondary">{expense.category}</td>
                <td className="p-4 text-electric-green text-right font-bold">{expense.value.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}</td>
                <td className="p-4 text-text-secondary text-center">
                  <div className="flex justify-center gap-4">
                    <button onClick={() => setEditingExpense(expense)} className="hover:text-data-blue transition-transform hover:scale-110"><FiEdit size={18} /></button>
                    <button onClick={() => handleDelete(expense.id)} className="hover:text-error transition-transform hover:scale-110"><FiTrash2 size={18} /></button>
                  </div>
                </td>
              </tr>
            ))}
        ...
        {/* ======================================================================== */}
        {/* 2. VISÃO DE CARDS PARA MOBILE (abaixo de md)                           */}
        {/* ======================================================================== */}
        <div className="md:hidden flex flex-col gap-4">
          {expenses.map((expense) => (
            <div key={expense.id} className="bg-dark-surface border border-dark-grid p-4 flex flex-col gap-3">
              <div className="flex justify-between items-start">
                <div>
                  <p className="text-text-primary font-bold text-lg">{expense.name}</p>
                  <p className="text-text-secondary text-xs">{expense.description}</p>
                </div>
                <p className="text-electric-green text-lg font-bold whitespace-nowrap">{expense.value.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}</p>
              </div>
              <div className="flex justify-between items-center text-sm border-t border-dark-grid pt-3">
                <div className="flex flex-col">
                  ...
                  <span className="text-text-primary">{formatarDataSegura(expense.date_created)}</span>
                </div>
                <div className="flex gap-4 text-xl">
                  <button onClick={() => setEditingExpense(expense)} className="text-text-secondary hover:text-data-blue"><FiEdit /></button>
                  <button onClick={() => handleDelete(expense.id)} className="text-text-secondary hover:text-error"><FiTrash2 /></button>
                </div>
              </div>
            </div>
          ))}
